...
const Message = ({ navigation, route }) => {
    ...
    navigation.setOptions({
      headerRight: () => (
        <View style={{ flexDirection: "row" }}>
          ...
          <Pressable
            onPress={() =>
              navigation.navigate("MessageSettings", { item: conver })
            }
          >
            <Ionicons
              name="list-outline"
      ...
      headerTitle: () => (
        ...
        >
          {conver.tag === "group" ? (
            <View style={{ width: "30%" }}>
              {conver.avatar ===
              "https://res.cloudinary.com/dq3pxd9eq/image/upload/group_avatar.jpg" ? (
                <Image
                  source={avatarGr}
  ...
  const fetchChats = async () => {
    try {
      if (conver && conver.messages) {
        const chatNew = await Promise.all(
          conver.messages.map(async (message) => {
            let senderName;
            let nameReply;
            if (
              conver.participants.find(
                (participant) => participant._id === message.senderId
              )
            ) {
              senderName = conver.participants.find(
                (participant) => participant._id === message.senderId
              );
            } else {
            ...
            if (message.replyMessageId !== null) {
              if (
                conver.participants.find(
                  (participant) =>
                    participant._id === message.replyMessageId.senderId
                )
              ) {
                nameReply = conver.participants.find(
                  (participant) =>
                    participant._id === message.replyMessageId.senderId
                );
              } else {
                const getUser = await getUserById(
                  message.replyMessageId.senderId
                );
  ...
  const fetchFriends = async () => {
    ...
    try {
      ...
      const newFriend = await Promise.all(
      ...
      if (isNewSocket === "new_message") {
        ...
        if (message && message.retrunMessage) {
          ...
          if (
            ...
            message.retrunMessage.senderId !== authUser._id
          ) {
  ...
  const handleDeleteMess = () => {
    ...
    handleDelete(
      `/chats/${messageSelected.chat._id}/delete`,
      "Thu hồi thành công"
    );
  };
  const handleDeleteMessByStatus = () => {
    setIsLoadXoa(true);
    handleDelete(
      `conversations/deleteOnMySelf/${conver.conversation._id}/${messageSelected.chat._id}`,
      "Xóa thành công"
    );
  };
  ...
  const appendIs = (formData, tag) => {
    ...
    formData.append(
      "replyMessageId",
      replyChat !== null ? replyChat.chat._id : null
    );
  };
  const setChat = async (dataChat) => {
    ...
    if (dataChat.replyMessageId) {
      nameReply = conver.participants.find(
        (participant) => participant._id === dataChat.replyMessageId.senderId
      );
      if (!nameReply) {
    ...
    if (dataChat.senderId !== authUser._id) {
      senderName = conver.participants.find(
        (sender) => sender._id === dataChat.senderId
      );
    }
    setChats((prevChats) => [
      ...prevChats,
  ...
  const handleSendMessage = async () => {
    if (textMessage) {
      send(
        conver._id,
        addMessage(textMessage, conver.tag, replyChat),
        "sendText"
      );
      setTextMessage(null);
    }
                  ...
                  {handleCheckShow(message) && (
                    <View
                      style={{
                        ...
                        flexDirection: "row",
                        alignItems: "center",
                        justifyContent: handleCheckIsSend(message)
                          ? "flex-end"
                          : "flex-start",
                      }}
                    >
                      ...
                      >
                        ...
                        {message.chat.replyMessageId === null ? (
                          <View></View>
                        ) : (
                          <View
                            style={{
            ...
            <View style={{ borderLeftWidth: 2, marginLeft: 10 }}>
              ...
              <View>
                {replyChat.chat.contents.map((content, i) => (
                  <View key={i}>{renderMessageContentReply(content)}</View>
                ))}
              </View>
            </View>
          </View>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtonContainer1}>
              ...
              {messageSelected?.sender?.profile?.name ===
