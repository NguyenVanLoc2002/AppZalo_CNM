...
import { Text, View, FlatList, Image, TextInput, ScrollView, Pressable } from 'react-native';
...
function SearchFriends({navigation}) {
  ...
  navigation.setOptions({
    headerRight: () => (
      <View style={{ flexDirection: 'row', justifyContent:'center' }}>
        <Pressable>
          <Ionicons name="qr-code" size={24} color="white" style={{ padding: 5, paddingStart: 15, marginEnd:15}} />
        </Pressable>
      </View>
    ),
    headerTitle: () => (
      <View style={{flexDirection:'row', alignItems: 'center'}}>
        <Ionicons name="search" size={24} color="grap" style={{ position: 'absolute', paddingStart:10 }} />
        <TextInput style={{ height: 45,width:300, paddingLeft: 40, borderRadius: 10, color: 'black', backgroundColor: 'white' }} placeholder='Tìm kiếm' placeholderTextColor={'gray'} />
      </View>
    ),
    headerStyle: {
  ...
  const renderApps = ({ item }) => (
    <Pressable style={{ height: 100, width: 100, justifyContent: 'center', alignItems: 'center' }}>
      <Image source={{ uri: item.url }} style={{ width: 50, height: 50, borderRadius: 25 }} />
  ...
  const renderHistory = ({ item }) => (
    <Pressable style={{ flexDirection: 'row' }}>
      <Ionicons name="search" size={20} color="grap" style={{ marginHorizontal: 10 }} />
      <Text>{item}</Text>
    </Pressable>
  );
    ...
    <View style={{ flex: 1 }}>
      <ScrollView>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: 10 }}>
          ...
          <Pressable><Ionicons name="close" size={24} color="gray" style={{ padding: 10 }} /></Pressable>
        </View>
        {/* FlatList component */}
        <View style={{ height: 100}} >
          ...
          />
          {/* <Pressable style={{ height: 100, width: 100, justifyContent: 'center', alignItems: 'center' }}>
            <Image source={{ uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAs4NFiTfZlx2QIuLHM065SpQbpcku32bmow&usqp=CAU' }} style={{ width: 50, height: 50, borderRadius: 25 }} />
            <Text style={{ textAlign: 'center' }}>Thêm</Text>
          </Pressable> */}
        </View>
        <View style={{ marginVertical: 10, height: 1, backgroundColor: 'gray', marginHorizontal: 10 }} />
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: 10 }}>
        ...
        <Pressable style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10 }}>
          ...
          <Ionicons name="chevron-forward" size={20} color="#0091FF" style={{ marginLeft: 5 }} />
        </Pressable>
      </ScrollView>
    </View>
  );
