...
function Chat({ navigation }) {
  ...
  const { conversations, getConversations, getConversationByID } =
    useConversation();
  ...
  const { isNewSocket, newSocketData, setIsNewSocket, setNewSocketData } = useSocketContext();
  ...
  useEffect(() => {
    ...
    const fetchSocket = async () => {
      ...
      if (isNewSocket === "add-to-group") {
        ...
        if (
          !listFriends.find((item) => item.conversation._id === data.group._id)
        ) {
      ...
      if (isNewSocket === "remove-from-group") {
        if (newSocketData.removeMembers) {
          let isChange = newSocketData; 
          if (isChange) {
            if (isChange.removeMembers?.includes(authUser._id)) {
              showToastSuccess(`Bạn đã bị xoá khỏi nhóm ${isChange.name}`);
              const updatedConversationList = listFriends.filter(
                (item) => item.conversation._id !== isChange.id
              );
      ...
      if (isNewSocket === "delete-group") {
        ...
        if(group){
          ...
          const updatedConversationList = listFriends.filter(
            (item) => item.conversation._id !== group.id
          );
          setListFriends(updatedConversationList);
          setIsNewSocket(null);
        }
